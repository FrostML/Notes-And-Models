...
class VGGNET():
    ...
    def net(self, input, num_class):
        ...
        for i in range(5):
            ...
            out_layer = self.conv_layers(out_layer, layers_num[i], num_filter=num_filters[i], name=name_layer)
        
        num_fc_size = [4096, 4096, num_class]
        fc_out = out_layer
        for x in range(3):
            fc_out = fluid.layers.fc(input=fc_out, size=num_fc_size[x], act='relu')
    ...
    def conv_layers(self, input, layer_num, num_filter, name):
        ...
        for i in range(layer_num):
            conv = fluid.layers.conv2d(conv, num_filter, filter_size=3, padding=1, stride=1, act='relu', param_attr=fluid.ParamAttr(name=name + "_weight" + str(i)), bias_attr=fluid.ParamAttr(name=name + "_bias" + str(i)))

        return fluid.layers.pool2d(conv, pool_size=2, pool_type='max', pool_stride=2)
...
def generate_images_and_labels(image_shape, label_shape):
    image = np.random.randint(low=0, high=255, size=image_shape).astype('float32')
    threshold = 0.5
    labels = []
...
if __name__ == "__main__":
    ...
    with fluid.program_guard(train_program, startup_program):
        data = fluid.data(name='data', shape=[-1, 3, 224, 224], dtype=np.float32)
        ...
        fluid.optimizer.SGD(learning_rate=0.01).minimize(fluid.layers.reduce_sum(loss))

    startup_program.random_seed=1
    exe.run(startup_program)
    # x = np.random.random(size=[1, 3, 224, 224]).astype('float32')
    # y = np.array([1]).reshape([1, 1]).astype('int64')
    loader = fluid.io.DataLoader.from_generator(feed_list=[data, label], capacity=6, use_double_buffer=True, iterable=True)
